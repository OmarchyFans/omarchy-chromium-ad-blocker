...
# ---- this plugin -------------------------------------------------------------
UPD_ID="fans.omarchy.chromium-ad-blocker"
UPD_NAME="Chromium Ad Blocker"
UPD_REPO="OmarchyFans/omarchy-chromium-ad-blocker"            # GitHub owner/repo the plugin is published from
UPD_BRANCH="main"        # branch whose manifest.json is "the published version"
UPD_SLUG="omarchy-adblock"            # cache lives in ~/.cache/<slug>/update-check.json
UPD_CONFIG="${XDG_CONFIG_HOME:-$HOME/.config}/omarchy-adblock/config.json"        # JSON file; "update_check": false turns the check off
...
post_update() {
  # Chromium loads the extension from this folder only at startup.
  local ans
  echo "Chromium loads the new extension only when it starts completely (all windows closed)."
  read -rp "Close and reopen Chromium now? Unsaved work in open tabs is lost. [y/N] " ans
  [[ $ans =~ ^[yY] ]] && "$UPD_DIR/bin/omarchy-adblock" restart || echo "Later: bin/omarchy-adblock restart"
}
# ------------------------------------------------------------------------------
