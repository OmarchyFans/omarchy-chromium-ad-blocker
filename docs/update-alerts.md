...
- Kind: bar-only widget (`BarWidget.qml`) that opens a terminal dashboard; helper `bin/omarchy-adblock` (bash) with `update-check`, `update-dismiss`, `update-run` delegating to `lib/update.sh`.
- Version places: `manifest.json`, `CHANGELOG.md`. The browser extension has its own `browser/extension/manifest.json` version, which is not the plugin version.
- Cache: `~/.cache/omarchy-adblock/update-check.json`. Opt-out: `"update_check": false` in `~/.config/omarchy-adblock/config.json` (the same file as the blocker's settings), or in the widget's `shell.json` entry.
- The widget checks on load and every six hours, shows a dot, and the next click opens the popup (Update…, Later, Open dashboard).
- Post-update step: Chromium loads the extension from the plugin folder only at a complete start, so the terminal offers `omarchy-adblock restart` (asks first, unsaved tabs are lost).
- No helper mismatch: `install.sh` symlinks the command and registers the native host by path into the plugin folder.
